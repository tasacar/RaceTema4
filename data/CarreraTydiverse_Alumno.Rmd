...
library(pacman)
p_load(dplyr,lubridate, tidyr, pacman)
...
```{r}
cran2<-cran2[order(cran2$package, cran2$ip_id), ]
```
...
```{r}
by_package%>%summarise(avg_size=mean(size, na.rm=TRUE))
```
...
pack_sum<-cran%>%
  group_by(package)%>%
  summarise(count=n(), unique=n_distinct(ip_id), countries=n_distinct(country), avg_bytes=mean(size, na.rm=TRUE), med_bytes=stats::median(size, na.rm=TRUE))
```
